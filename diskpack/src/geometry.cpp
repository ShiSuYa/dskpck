...
void RadiusRegion::split(std::vector<RadiusRegion> &regions, 
  ...
  for (size_t i = 0; i < k; ++i) {
    ...
    auto u =
        (i < k - 1 ? initial.lower() + (i + 1) * sub_width : initial.upper());
...
bool RadiusRegion::isNarrowEnough(BaseType lower_bound) const {
  return !std::any_of(
      intervals.begin(), 
      intervals.end(),
      [&lower_bound](const Interval &x) { return width(x) > lower_bound; });
}
...
bool RadiusRegion::isTooWide(BaseType upper_bound) const {
  return std::any_of(
      intervals.begin(), 
      intervals.end(),
      [&upper_bound](const Interval &x) { return width(x) > upper_bound; });
}
...
IntervalPair
SpiralOp::operator*(const IntervalPair &vec) const noexcept {
  return IntervalPair{vec.first * x - vec.second * y,
                      vec.second * x + vec.first * y};
}

SpiralOp SpiralOp::operator*(
    const SpiralOp &other) const noexcept {
  return SpiralOp{other.x * x - other.y * y,
                  other.y * x + other.x * y};
}
